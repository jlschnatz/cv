...
format_pkg_title <- function(title) {
  close_loc <- str_locate_all(title, "\\}") %>%
    ...
    as_tibble() %>%
    mutate(
      first = case_when(start == min(start) ~ "first"),
...
cite_incollection <- function(ref_info) {
  ...
  ref_info %>%
    ...
    dplyr::select(
      full_author, year, title, full_editor, booktitle, edition, pages,
      publisher, doi
    ) %>%
    glue::glue_data(
...
cite_info <- ref_info %>%
    ...
    cite <- cite_info %>%
      glue::glue_data(
        "{full_author} ({full_date}). *{title}{ifelse(is.na(title), '', paste0(': ', title))}* [{titleaddon}]. {eventtitle}, {venue}."
      )
...
cite_report <- function(ref_info) {
  ...
  cite_info <- ref_info %>%
    dplyr::mutate(
      ...
      full_title = dplyr::case_when(
        is.na(title) ~ title,
        TRUE ~ paste0(title, ": ", title)
      )
    )
...
cite_manual <- function(ref_info) {
  ...
  new_info %>%
    ...
    mutate(
      url = case_when(
        str_detect(note, "https") ~ note,
        TRUE ~ url
      ),
      note = case_when(
        is.na(url) ~ "Internal R package",
        TRUE ~ note
      ),
      url = map_chr(
        url,
        function(u) {
          if (is.na(u) | !str_detect(u, ",")) {
            return(u)
          }
...
format_bib <- function(bib, emphasize = "Thompson, W. J.") {
  all_bib <- bib2df::bib2df(bib) %>%
    ...
    dplyr::mutate(
      citation = purrr::map2_chr(category, info, format_citation),
